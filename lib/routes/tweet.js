  ...
  async (req, res) => {
    try {
      ...
      if (user.likes.includes(req.params.id)) {
        const index = user.likes.indexOf(req.params.id)
        if (index !== -1) {
  ...
  async (req, res) => {
    try {
      ...
      if (user.retweets.includes(req.params.id)) {
        ...
        const index = user.retweets.indexOf(req.params.id)
        if (index !== -1) {
          user.retweets.splice(index, 1)
        }
        const tweetIndex = tweet.retweets.indexOf(req.user._id)
        if (tweetIndex !== -1) {
          tweet.retweets.splice(tweetIndex, 1)
        }
        user.save()
        tweet.save()
        ...
        const retweet = await Tweet.create(newRetweet)
