...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      user.save()
      res.send({ success: true, msg: 'list created successfully', list })
    } catch (err) {
...
router.put(
  ...
  async (req, res) => {
    ...
    Object.keys(updateList).forEach((key) =>
      updateList[key] === undefined || updateList[key] === ''
    ...
    try {
      ...
      if (list.user.toString() == req.user._id) {
        List.findByIdAndUpdate(req.params.id, updateList, {
          ...
          .catch((err) =>
            res
              .status(500)
              .send({ success: false, msg: 'unknown server error' }),
          )
      } else {
  ...
  async (req, res) => {
    try {
      const list = await List.findById(req.params.id)
        ...
        .populate('users', 'username name profileImg')
      const users = []
      ...
      const listTweets = await Tweet.find()
        ...
        .in(users)
        .sort({ _id: -1 })
        .populate('user', 'username name _id profileImg')
        .populate({
          path: 'parent',
