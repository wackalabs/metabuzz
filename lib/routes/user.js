...
router.get('/:username', async (req, res) => {
  ...
  try {
    let user
    if (
      (decoded && decoded.username == req.params.username) ||
  ...
  async (req, res) => {
    ...
    try {
      ...
      const user = await User.findOne(
        { username: req.user.username },
        { lists: 1 },
      ).populate({ path: 'lists', model: 'List', select: 'name banner' })
  ...
  async (req, res) => {
    try {
      ...
      Object.keys(updateUser).forEach((key) =>
        updateUser[key] === undefined || updateUser[key] === ''
          ? delete updateUser[key]
          : null,
      )

      User.findOneAndUpdate({ username: req.user.username }, updateUser, {
    ...
    try {
      const user = await User.findOne(
        ...
        .populate({
          path: 'followers',
          model: 'User',
          select: 'name username profileImg description',
        })
        .populate({
          path: 'following',
          model: 'User',
  ...
  async (req, res) => {
    ...
    try {
      const users = await User.find(
        ...
        { profileImg: 1, name: 1, username: 1 },
      ).limit(4)
